...
import { useGetSaleUsersQuery } from "@/services/sales/sale.service";
import { SaleUserTable } from "./saleuserTable";

const SaleUser: React.FC = () => {
  ...
  useEffect(() => {
    if (saleUser) {
      const totalPages = Math.ceil(saleUser.count / pagination.pageSize);
...
<div className="flex items-center justify-center h-screen">
  ...
  const breadcrumbs = [{ label: "Home", path: "/" }, { label: "Sale User" }];
    ...
    <>
      ...
      <div className="bg-white rounded-lg p-5">
        <div className="flex justify-between">
          <span className="text-xl font-bold">Sale User</span>
          <Link to="/sales-user/add/">
            <Button className="flex text-white rounded-md bg-blue-700 hover:bg-blue-400">
              <LuUserPlus2 className="mr-1 text-lg" />
              <span>Add Sale User</span>
            </Button>
          </Link>
        </div>
        <SaleUserTable
          saleUser={saleUser?.results as any}
